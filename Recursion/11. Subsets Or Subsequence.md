```js
Given an integer array `nums` of **unique** elements, return _all possible subsets (the power set).
...
In your base case condition satisfies return true otherwise false and whenever you do a function call put it in a if block and check if the function call returned true, if true then return true. What this will allow us to do is that when our condition gets satisfied the rest code won't execute because that function which returned true will keep returning true until stack is empty.
```cpp
#include <bits/stdc++.h> 
using namespace std;
