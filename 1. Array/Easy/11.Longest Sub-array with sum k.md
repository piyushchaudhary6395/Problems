...
## Better
```js
we use hash map, for its key we use the arrays sum (prefix sum) till `i` and for its value `i` index.
...
```js
if we use map[] syntex to insert a value and the kay of that value already exists in the map the it will override the value so instead of that we can use `insert` function using this if the key exists then the value will not be overwritten.
